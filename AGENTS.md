...
- `MainWindow`: WPF-only integration for AvalonEdit, the document inspector, dialogs, composition-mode WebView2, keyboard input, drag/drop, visual selection/movement/resizing, same-parent Arrange commands, opacity gestures, and document persistence. Inspector/property integration stays in `MainWindow.Inspector.cs`; visual-editing integration stays in `MainWindow.VisualEditing.cs`; template/recovery/Auto Save integration stays in `MainWindow.Persistence.cs`.
