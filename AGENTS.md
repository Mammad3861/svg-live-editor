...
SvgLiveEditor is an open-source Windows desktop application for editing UTF-8 SVG/XML source and viewing a live, security-restricted preview. Version 0.7.1 stabilizes the editor UX with an app-owned AvalonEdit context menu, visible accessible Property help, compact same-parent layer information, focus-aware Properties Undo/Redo, and a modern host-owned selection overlay while preserving all v0.7 Stage 2 Arrange/opacity behavior, Stage 1 resize handles, v0.6 selection/movement, simple-text measurement, the installed-font picker, templates, recovery, validation-gated Auto Save, Inspector, preview, PNG sharing, file drop, and Pan functionality. Source text remains the single source of truth. It does not cover a full Layers panel, rotation handles, text resize handles, multi-selection, element creation/deletion/duplication, alignment, snapping, grouping, inline canvas text editing, complex text layout, path-node editing, explicit image/PDF file export, installers, updates, cloud features, full Figma-style editing, BPMN, or AI generation.
...
- `MainWindow`: WPF-only integration for AvalonEdit, the document inspector, dialogs, composition-mode WebView2, keyboard input, drag/drop, visual selection/movement/resizing, same-parent Arrange commands, opacity gestures, and document persistence. Source-editor menu integration stays in `MainWindow.SourceEditor.cs`; Inspector/property integration stays in `MainWindow.Inspector.cs`; visual-editing integration stays in `MainWindow.VisualEditing.cs`; template/recovery/Auto Save integration stays in `MainWindow.Persistence.cs`.
...
- Arrange commands may reorder only exact current full-source spans for eligible paintable siblings under the same indexed parent. Never cross parent/container boundaries or rewrite the whole document. Preview context-menu ordering must remain navigation-token-, source-revision-, and opaque-selection-bound and stale-safe.
