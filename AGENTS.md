...
SvgLiveEditor is an open-source Windows desktop application for editing UTF-8 SVG/XML source and viewing a live, security-restricted preview. Version 0.7.0 Stage 1 adds host-owned resize handles for a deliberately small set of basic shapes while preserving v0.6 selection, movement, simple-text measurement, the installed-font picker, templates, recovery, validation-gated Auto Save, Inspector, preview, PNG sharing, file drop, and Pan functionality. Source text remains the single source of truth. It does not cover rotation handles, text resize handles, multi-selection, element creation/deletion/duplication, layers, alignment, snapping, grouping, inline canvas text editing, complex text layout, path-node editing, explicit image/PDF file export, installers, updates, cloud features, full Figma-style editing, BPMN, or AI generation.
