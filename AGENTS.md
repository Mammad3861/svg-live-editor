...
SvgLiveEditor is an open-source Windows desktop application for editing UTF-8 SVG/XML source and viewing a live, security-restricted preview. Version 0.9.0 Visual Authoring is a complete standalone release with bounded creation of basic SVG elements and empty groups, exact-subtree duplication/deletion, and explicit conservative reparenting into, out of, and between existing groups. It preserves the v0.8 Layers & Groups workspace, v0.7.1 editor UX, Arrange/opacity, resize handles, selection/movement, text measurement, installed-font picker, templates, recovery, validation-gated Auto Save, Inspector/Structure, preview, PNG sharing, file drop, and Pan functionality. Source text remains the single source of truth. Version 0.10.0 Visual Composition is reserved for multi-selection, moving multiple selected elements, Group/Ungroup, alignment/distribution, basic snapping, and evaluation of safe basic path bounding-box resize after the composition architecture is stable. Version 1.0.0 is the Stable Release/stabilization milestone. Rotation, text resize handles, inline canvas text editing, complex text layout, path-node editing, explicit image/PDF file export, installers, updates, cloud features, full Figma-style editing, BPMN, and AI generation remain out of the current scope.
...
- Authoring may create only the fixed app-owned `rect`, `circle`, `ellipse`, `line`, direct `text`, and empty `g` kinds with bounded host-derived defaults. Duplicate/delete/reparent must operate on exact current full-source spans, preserve unrelated bytes, revalidate candidates, and commit one Undo unit. Generated IDs must be deterministic and unique. Rewrite only analyzed local references within a copied subtree; reject missing or ambiguous targets. Reject cross-parent moves across transforms, presentation/inheritance, effects, namespace ambiguity, definitions, locks, cycles, or stale identities rather than changing meaning.
