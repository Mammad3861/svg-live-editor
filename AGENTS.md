...
SvgLiveEditor is an open-source Windows desktop application for editing UTF-8 SVG/XML source and viewing a live, security-restricted preview. Version 0.10.0 Stage 1 adds bounded Preview/Layers multi-selection with one primary/anchor, atomic multi-object movement and nudge, conservative same-parent Group/Ungroup, visual-bounds alignment/distribution, and optional CSS-pixel-based object/canvas-center snapping to the completed Stage 0 Visual Authoring stabilization. It preserves presentation-attested latest-wins Preview rendering, explicit root/selected-context creation, accessible Layers disclosure, source-owned Unicode friendly layer names, focus-aware editing shortcuts, exact-subtree duplicate/delete/reparent, Arrange/opacity, single-selection resize handles, text measurement, templates, recovery, validation-gated Auto Save, PNG sharing, file drop, and Pan. Source text remains the single source of truth. Later v0.10 polish may evaluate safe basic path bounding-box resize and safely allowing supported geometry to cross the root canvas edge. Version 1.0.0 is the Stable Release/stabilization milestone. Multi-selection resize, rotation, text resize handles, inline canvas text editing, complex text layout, path-node editing, explicit image/PDF file export, installers, updates, cloud features, full Figma-style editing, BPMN, and AI generation remain out of the current scope.
