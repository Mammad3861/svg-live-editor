...
SvgLiveEditor is an open-source Windows desktop application for editing UTF-8 SVG/XML source and viewing a live, security-restricted preview. Version 0.11.0 adds revision-bound, explicit-origin synchronization across Source, Preview, Layers/Structure, Properties, and the primary selection while keeping the exact Source text authoritative. It preserves the v0.10.0 Stage 1 bounded Preview/Layers multi-selection with one primary/anchor, atomic multi-object movement and nudge, conservative same-parent Group/Ungroup, visual-bounds alignment/distribution, optional CSS-pixel-based object/canvas-center snapping, presentation-attested latest-wins Preview rendering, explicit root/selected-context creation, accessible Layers disclosure, source-owned Unicode friendly layer names, focus-aware editing shortcuts, exact-subtree duplicate/delete/reparent, Arrange/opacity, single-selection resize handles, text measurement, templates, recovery, validation-gated Auto Save, PNG sharing, file drop, and Pan. Later v0.10 polish may evaluate safe basic path bounding-box resize and safely allowing supported geometry to cross the root canvas edge. Version 1.0.0 is the Stable Release/stabilization milestone. Multi-selection resize, rotation, text resize handles, inline canvas text editing, complex text layout, path-node editing, explicit image/PDF file export, installers, updates, cloud features, full Figma-style editing, BPMN, and AI generation remain out of the current scope.
...
- `Services`: testable SVG validation, explicit source-change-origin and current-revision synchronization policy, validated source and visual-geometry indexing, bounded revision-safe multi-selection, host-side topmost-first Layers projection with opaque session identities, conservative visibility and inherited lock policy, bounded element creation, exact-subtree duplicate/delete, minimal contiguous Group/neutral Ungroup, local reference remapping, guarded cross-parent reparenting, browser-backed simple-text measurement, installed-font enumeration, coordinate mapping, conservative hit testing, atomic multi-move, bounded single resize, visual-bounds alignment/equal-gap distribution, CSS-pixel object snapping, exact-span same-parent artwork/group ordering and layer-position presentation, conservative opacity analysis, minimal attribute edits, focus-aware shortcut policies, secure preview/PNG bridge generation, bounded clipboard/drag payloads, bounded UTF-8 file access, embedded template loading, recovery retention, safe path/atomic Auto Save policy, drag-file cleanup, debounce behavior, sample loading, and unsaved-change policy.
...
- `MainWindow`: WPF-only integration for AvalonEdit, Layers/Structure, Properties, dialogs, composition-mode WebView2, keyboard input, external file drop, internal Layers ordering/reparent drag/drop, basic element authoring, visual selection/movement/resizing, same-parent Arrange commands, opacity/visibility/lock gestures, and document persistence. Source-editor menu integration stays in `MainWindow.SourceEditor.cs`; Layers/Structure/property/authoring integration stays in `MainWindow.Inspector.cs`; visual-editing integration stays in `MainWindow.VisualEditing.cs`; template/recovery/Auto Save integration stays in `MainWindow.Persistence.cs`.
...
- Visual hit testing must remain host-side over the current validated source index. Unsupported topmost artwork must not permit click-through: localize only bounds that can be derived conservatively, and otherwise fail closed. Multi-selection is limited to 128 opaque source identities with one primary/anchor and must reconcile by current validated revision without trusting duplicate authored IDs. Keep visual messages exact-schema, navigation-token-bound, source-revision-bound, coordinate-bounded, and stale-safe; the app-owned overlay must contain only host-generated bounded geometry and at most one temporary snap guide per axis.
...
- Authoring may create only the fixed app-owned `rect`, `circle`, `ellipse`, `line`, direct `text`, and empty `g` kinds with an explicit SVG-root or selected-context destination and bounded host-derived defaults. Duplicate/delete/reparent/Group/Ungroup must operate on exact current full-source spans, preserve unrelated bytes, revalidate candidates, and commit one Undo unit. Group accepts only 2–128 current contiguous eligible siblings under one SVG/group parent. Ungroup accepts only a current non-empty `<g>` with no attributes and no direct group-targeting animation; ambiguous metadata or rendering semantics fail closed. Generated technical IDs must be deterministic and unique. Rewrite only analyzed local references within a copied subtree; reject missing or ambiguous targets. Friendly layer rename edits only an optional bounded, XML-safe `data-name` attribute, preserves Unicode exactly, and never changes the technical `id` or fragment references. Recheck current source spans/revision/locks and fail closed for illegal, stale, or ambiguous edits. Reject cross-parent moves across transforms, presentation/inheritance, effects, namespace ambiguity, definitions, locks, cycles, or stale identities rather than changing meaning.
- Arrange may reorder only exact current full-source spans for eligible artwork or `<g>` siblings under the same indexed parent and must never infer a parent change. Alignment/distribution require the complete 2+/3+ set to be measurable, movable, unlocked, current, and under one compatible immediate parent; all geometry edits are combined and validated before one Undo unit. Object snapping may use only current visible, unlocked, safely movable sibling bounds and canvas centers with a four-CSS-pixel threshold mapped through the current image coordinate transform; full-canvas backgrounds and ambiguous ties are suppressed, it mutates no source until release, and its host guides never enter PNG output. Layers reparenting must be an explicit before/after/inside operation through the guarded reparent service. Preview context-menu ordering must remain navigation-token-, source-revision-, and opaque-selection-bound and stale-safe.
- Layers visibility may add only a standard direct `display="none"` when `display`, `visibility`, inline style, and visibility animation ownership are unambiguous. Remove it only when the current session owns that hidden state; never overwrite authored visibility semantics. Locks are session-only, inherited through layer groups, never serialized into SVG/XML, and guard Create, Duplicate, Delete, Reorder, Reparent, Move, Resize, Nudge, Opacity, and Properties without becoming a security boundary; Source editing stays available.
...
- Direct preview drag must remain a paired, token-bound trusted-page gesture. Require an exact `Ctrl`-only primary left-button drag on the isolated image (`Alt`-only may remain a compatibility alias), Pan-off arbitration, bounded coordinates, the Windows drag threshold, and a matching host-side armed gesture before requesting the existing validated PNG pipeline. Space/Pan mode/middle drag remain Pan, plain left drag remains visual movement, Shift+click remains multi-selection, and conflicting modifiers fail closed.
