...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.6.0 adds a secure Visual Editing Foundation for selecting and moving basic shapes and a conservative subset of simple text in Live Preview while keeping the exact UTF-8 SVG source as the single source of truth.
...
- Properties inspector for common safe attributes, shape geometry, and text typography. The editable `font-family` picker suggests installed Windows fonts plus common generic fallbacks while preserving typed comma-separated stacks. Changes are minimal source edits, participate in Undo/Redo, and never reserialize the full XML document; path `d` remains read-only.
...
The trusted interaction bridge has automated browser integration coverage, including Chromium metrics for English and Persian text. Physical shape/text selection and movement, font changes, inbound drops, `Alt`+artwork drag and Drag Image compatibility, Ctrl+Wheel, the app-owned right-click menu, focus-sensitive Ctrl+C, clipboard paste compatibility, touchpad scrolling, and the four drag-to-pan methods should still be confirmed on the target machine for each release.
...
Version 0.6 provides a deliberately small direct-manipulation foundation: validated, untransformed `rect`, `circle`, `ellipse`, and `line` elements with unitless or `px` geometry can be selected and moved. A `<text>` element is also selectable and movable when it contains short direct plain text only, has one finite unitless/`px` `x` and `y`, a positive unitless/`px` font size, supported local font/style/weight/anchor/direction/bidi values, and no transform, effects, stroke, preserved/complex whitespace, layout offsets, nested elements, or complex typography. Nested `tspan`, `textPath`, animation, coordinate lists, percentages and other units are deliberately not approximated. `polygon`, `polyline`, paths, groups, resize, rotation, multi-selection, snapping, creation, deletion, reordering, inline canvas text editing, and path-node editing remain unsupported. Unsupported elements remain available in Source and Inspector. Explicit PNG/PDF **file export**, an installer, and automatic updates are also not provided.

| Preview selection category | v0.6 behavior |
| --- | --- |
| `rect`, `circle`, `ellipse`, `line` | Selectable and movable when geometry and ancestors meet the conservative unit/transform/effect rules. |
...
| `path`, `polygon`, `polyline` | Not movable. Conservatively indexed bounds prevent selecting lower artwork through a topmost unsupported element when reliable bounds are available. |
...
Direct resize handles, including text resize handles, are deliberately not part of v0.6 and are planned for a later visual-editing release.

## Roadmap

Future work may include improved AvalonEdit mixed RTL/BiDi caret and glyph behavior, richer template organization, configurable recovery timing, resize/rotation handles, multi-selection, snapping, path editing, explicit PNG/PDF file export, and an installer with automatic updates. Auto Save remains deliberately validation-gated and is not a replacement for version control or backups.
...
```powershell
Get-FileHash .\SvgLiveEditor-v0.6.0-win-x64.zip -Algorithm SHA256
```
