...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.8.0 adds a native Layers & Groups workspace with clear paint order, nested groups, conservative visibility controls, and session-only locks while preserving the v0.7.1 editor UX and security boundaries. The exact UTF-8 SVG source remains the single source of truth.
...
Version 0.8.0 adds the Layers/Groups foundation without broadening direct geometry editing. Validated, untransformed `rect`, `circle`, `ellipse`, and `line` elements with unitless or `px` geometry can be selected, moved, and resized; simple direct `<text>` remains selectable and movable, and conservatively bounded `path`, `polygon`, and `polyline` remain inspection-only. Existing `<g>` elements are expandable Layers containers and paint-order units. Eligible artwork and groups can reorder only among siblings under the same indexed parent. Explicit move into/out of a group, group creation/ungrouping, rotation, multi-selection, snapping, creation, deletion, duplication, alignment, inline canvas text editing, and path-node editing remain out of scope. Unsafe or unbounded artwork remains blocked. Explicit PNG/PDF **file export**, an installer, and automatic updates are also not provided.

| Preview/Layers category | v0.8.0 behavior |
| --- | --- |
...
| `g` | Expandable in Layers, reorderable as one same-parent paint-order unit, and able to carry conservative visibility/session-lock state. Preview still selects eligible children rather than exposing an SVG group DOM. |
...
Text resize handles are deliberately not part of v0.8; use the validated `font-size` property instead. The minimum shape dimension is 0.01 SVG user units, and crossing an anchored edge clamps at that value rather than flipping the element.
...
```powershell
Get-FileHash .\SvgLiveEditor-v0.8.0-win-x64.zip -Algorithm SHA256
```
