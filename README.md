...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.10.0 Stage 1 adds bounded multi-selection, atomic multi-object movement, conservative Group/Ungroup, alignment/distribution, and optional object snapping on top of the presentation-attested Stage 0 authoring foundation. The exact UTF-8 SVG source remains the single source of truth.
...
Version 0.10.0 Stage 1 implements bounded multi-selection, atomic multi-object movement/nudge, conservative Group/Ungroup, six alignment modes, equal-gap distribution, and optional object/canvas-center snapping while preserving the Stage 0 latest-wins mounted preview and source-authoritative authoring safeguards. Properties and Source continue to follow one primary item; multi-selection resize, arbitrary transforms/rotation, path movement/resize, live artwork mutation during pointer movement, inline canvas text editing, rulers, manual guides, and advanced smart-layout suggestions are not included. Unsafe or ambiguous operations fail closed. Explicit PNG/PDF **file export**, an installer, and automatic updates are also not provided.
