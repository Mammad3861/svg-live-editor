...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.7.0 Stage 2 adds bounded same-parent paint ordering and a conservative element-opacity control while preserving the secure Stage 1 resize handles, v0.6 selection, movement, and simple-text editing. The exact UTF-8 SVG source remains the single source of truth.
...
- **Arrange** provides Bring to Front, Bring Forward, Send Backward, and Send to Back for eligible paintable siblings under the same parent. The shortcuts are `Ctrl+Shift+]`, `Ctrl+]`, `Ctrl+[`, and `Ctrl+Shift+[`. Commands move exact indexed element spans, preserve comments, whitespace, UTF-8/Persian text, IDs, references, and one-step Undo, and never cross group/container boundaries. They are available from the top-level menu, the Inspector tree context menu, and the app-owned Preview context menu when its revision-bound selection is current.
...
Version 0.7 Stage 2 keeps the deliberately small direct-manipulation foundation: validated, untransformed `rect`, `circle`, `ellipse`, and `line` elements with unitless or `px` geometry can be selected, moved, and resized through bounded handles. A `<text>` element is selectable and movable when it contains short direct plain text only, but its size remains a `font-size` Properties edit and it receives no resize handles. Eligible `rect`, `circle`, `ellipse`, `line`, `text`, and reliably bounded inspect-only `path`, `polygon`, and `polyline` elements can change paint order only among paintable siblings under the same parent and can use the conservative direct-opacity control. Nested `tspan`, `textPath`, animation, coordinate lists, percentages and other units are deliberately not approximated. Groups, rotation, multi-selection, snapping, creation, deletion, duplication, a full Layers panel, alignment, grouping, inline canvas text editing, and path-node editing remain unsupported for direct manipulation. Unsafe or unbounded artwork remains blocked. Explicit PNG/PDF **file export**, an installer, and automatic updates are also not provided.

| Preview selection category | v0.7 Stage 2 behavior |
| --- | --- |
...
Text resize handles are deliberately not part of v0.7; use the validated `font-size` property instead. The minimum shape dimension is 0.01 SVG user units, and crossing an anchored edge clamps at that value rather than flipping the element.
