...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.7.1 stabilizes the editor UX with a native Source context menu, a clearer modern selection overlay, compact same-parent layer feedback, visible Property help, and focus-aware Properties Undo/Redo while preserving all v0.7 visual editing and security boundaries. The exact UTF-8 SVG source remains the single source of truth.
...
```powershell
Get-FileHash .\SvgLiveEditor-v0.7.1-win-x64.zip -Algorithm SHA256
```
