...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.11.0 adds a revision-bound Source synchronization foundation to the completed v0.10 visual-composition tools. The exact UTF-8 SVG source remains the single source of truth.
...
- Native **Layers** and **Structure** tabs share one validated source index. Layers shows visual artwork and nested `<g>` containers with the frontmost item first while filtering definitions such as gradients, markers, masks, filters, and metadata; Structure preserves the complete XML hierarchy. Expandable groups use accessible native disclosure chevrons with persistent expansion state and standard mouse/Left/Right behavior. Plain click selects one item, Ctrl+click toggles a row, and Shift+click selects a bounded contiguous range within one visible sibling container. Preview Shift+click toggles an item. Up to 128 items are retained through opaque revision-bound identities; one primary/anchor stays synchronized with Properties and Source without pretending that AvalonEdit can display disjoint text ranges.
- The compact **Add** control and the Layers/Structure context menus create bounded `rect`, `circle`, `ellipse`, `line`, `text`, and empty `g` elements without raw XML input. The user explicitly chooses either the SVG root or the selected group/sibling context; the app never guesses between them. New geometry is derived from the current safe canvas/viewBox, selected immediately in both trees and Properties, and committed as one Undo operation. F2 or **Rename Layer** edits only the optional source-owned `data-name` label; Persian, English, mixed Unicode, spaces, and safe punctuation are supported, while technical SVG `id` values and fragment references remain untouched. Clearing the field removes `data-name`. `Ctrl+D` duplicates the selected layer; Delete and Backspace remove an eligible selection when Layers, Structure, Preview, or a non-editing Properties surface has focus, with confirmation for non-empty groups. These shortcuts do not take over Source, property fields, editable font/combobox fields, inline rename, or IME composition.
...
- The v0.11 synchronization foundation binds each current valid Source revision to its matching Preview, Layers/Structure index, Properties state, and primary selection. Explicit Source, Properties, visual-command, Undo/Redo, and document-load origins prevent feedback loops. Caret-only Source synchronization changes inspection only; it never changes source text, Undo history, Modified state, or keyboard focus. Stale work is ignored; invalid XML preserves the exact typed source and last valid Preview while visual and Properties mutations remain disabled.
...
- A visible Select tool (`V`) synchronizes Live Preview selection with Layers/Structure, Properties, and the primary source start tag. A safely movable multi-selection of `rect`, `circle`, `ellipse`, `line`, and measured direct `<text>` moves together by drag or Arrow keys (1 SVG unit; `Shift`+Arrow uses 10). The complete set is rejected if any member is stale, locked, ambiguous, or unsupported; no safe subset is silently moved. Host-owned primary/secondary outlines follow the candidate move and one release/nudge creates one Undo unit. Resize handles remain deliberately single-selection-only and retain the existing shape behavior. Source is never rewritten on every pointer move.
- **Arrange** retains same-parent Bring to Front/Forward and Send Backward/Back, and adds **Group** (`Ctrl+G`), conservative **Ungroup** (`Ctrl+Shift+G`), six alignment commands, and horizontal/vertical distribution. Group accepts only 2–128 current contiguous siblings under one SVG/group parent, writes one real `<g>` wrapper into Source, and makes that new expandable group primary. Ungroup accepts only a current non-empty neutral `<g>` with no attributes or group-targeting animation. Both shortcuts route from Preview, Layers, Structure, and other non-editable window surfaces; rejected commands explain the exact reason in the status bar. Alignment uses validated visual bounds and the combined selection's left/right/top/bottom or center; distribution keeps the two geometry-sorted outer objects fixed and makes visual gaps equal. Layout requires safely measurable, movable, unlocked items under one compatible parent coordinate system and commits one minimal Undo edit; no-op boundaries create no edit. Layers drag/drop remains the separate explicit before/after/inside cross-parent operation and fails closed when inherited semantics are ambiguous.
- **View > Snap to objects** is enabled by default and saved per user. During a safe visual move, visible, unlocked, safely movable sibling left/center/right and top/center/bottom bounds plus the canvas/viewBox centers are candidates within 4 CSS pixels. The smallest correction wins; ambiguous ties and full-canvas background siblings are ignored. The threshold is converted through the current Fit/manual zoom mapping, and at most one host-owned guide per axis is shown. Guides clear without editing source and are excluded from PNG output.
- Each Layers row has app-owned visibility and lock controls. Visibility toggling adds only a standard `display="none"` to an otherwise unambiguous element and removes it only while that hidden attribute is known to belong to the current editor session. Existing authored `display`, `visibility`, inline style, or animation ownership is never overwritten; affected controls are disabled with an explanation. Hidden state remains in the SVG and therefore survives Save/reopen. Locks are deliberately session-only because SVG has no portable editor-lock attribute: an inherited group lock blocks Create into the group, Duplicate, Delete, Group/Ungroup, Move, Resize, Nudge, layout, Arrange, reorder/reparent, Opacity, and supported Properties edits, but Source editing remains available.
...
- A visible Pan toolbar mode (`H` to toggle and `Escape` to exit). Overflowing previews can also be panned temporarily with Space+left drag or middle-button drag. Pan mode and Space+drag take priority over selection and image sharing.
...
- Use `Ctrl`+left-drag on the rendered artwork to drag a PNG into Explorer, Telegram Desktop, Paint, or another Windows drop target; `Alt`+left-drag remains a temporary compatibility alias. Plain left drag moves selected supported artwork, while Shift+click toggles multi-selection and Space+left/middle drag pans. Starting on checkerboard space, combining modifiers, clicking without crossing the Windows drag threshold, or using a Pan gesture does not export. The visible **Drag Image** toolbar control remains an accessible fallback. Both entry points use the same validated full-artwork render and provide a real PNG file plus PNG/bitmap formats while preserving transparency.
...
The trusted interaction bridge has automated browser integration coverage, including Chromium metrics for English and Persian text, a real pointer-driven rectangle resize, and revision/selection-bound context-menu requests. Physical multi-selection in Preview/Layers, multi-drag/nudge, Group/Ungroup, each alignment/distribution command, snap guides at Fit/manual zoom, Add/Duplicate/Delete, Layers reparenting, locks, resize handles, opacity, font changes, inbound/drop-out sharing, Ctrl+Wheel, focus-sensitive Ctrl+C, touchpad scrolling, and Pan should still be confirmed on the target machine for each release.
...
Validated SVG is UTF-8/Base64 encoded into an HTML `<img>` data URL. Raw SVG is never inserted into host markup. The restrictive CSP authorizes one fixed app-owned interaction script only by its exact SHA-256 hash. Preview pointer input carries no authored SVG identity: .NET performs hit testing and maintains at most 128 opaque revision-bound selections. Host-to-page overlay messages use an exact bounded schema for host-generated geometry, one primary selection, and at most two temporary snap guides; arbitrary multi-selection or source-edit messages are not accepted. Resize remains bound to one current random selection/gesture ID and physical button state. Text measurement receives only bounded validated plain text and typography. PNG copy draws only the isolated image to a transparent off-screen canvas, so all outlines, handles, and guides are excluded. Native browser zoom stays at 100%; scripts, host objects, arbitrary messages, permissions, downloads, pop-ups, external navigation/resources, developer tools, and browser menus remain blocked. Invalid edits keep the last valid preview visible and disable visual editing.
...
Version 0.11.0 keeps the v0.10 bounded multi-selection, movement, Group/Ungroup, alignment/distribution, and snapping scope, and adds predictable revision-bound synchronization among Source, Preview, Layers/Structure, Properties, and the primary selection. Multi-selection still exposes only its primary item to Source and Properties; AvalonEdit is never made to display fake disjoint selections. Multi-selection resize, arbitrary transforms/rotation, path movement/resize, live artwork mutation during pointer movement, inline canvas text editing, rulers, manual guides, and advanced smart-layout suggestions are not included. Unsafe or ambiguous operations fail closed. Explicit PNG/PDF **file export**, an installer, and automatic updates are also not provided.
...
| Preview/Layers category | v0.10.0 Stage 1 behavior |
| --- | --- |
...
| `g` | Existing and created groups remain expandable/reorderable/reparentable. Group wraps only contiguous same-parent selections. Ungroup removes only a non-empty wrapper with no attributes or group-targeting animation; otherwise it explains the fail-closed rejection. Preview still exposes no SVG group DOM. |
...
Text and multi-selection resize handles are deliberately not part of v0.10.0 Stage 1; use the validated `font-size` property for text. The minimum shape dimension is 0.01 SVG user units, and crossing an anchored edge clamps at that value rather than flipping the element.
...
```powershell
Get-FileHash .\SvgLiveEditor-v0.11.0-win-x64.zip -Algorithm SHA256
```
