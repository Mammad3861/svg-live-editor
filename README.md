...
SvgLiveEditor is an open-source Windows desktop application for editing SVG/XML source and viewing the result immediately. Version 0.10.0 Stage 0 stabilizes the v0.9.0 Visual Authoring release with presentation-attested latest-wins Preview rendering, explicit creation destinations, accessible Layers disclosure, Unicode friendly layer names, and focus-aware Delete/Backspace. The exact UTF-8 SVG source remains the single source of truth.
...
Version 0.10.0 Stage 0 stabilizes the complete v0.9.0 Visual Authoring scope: valid Preview updates are latest-wins across rapid authoring/Undo/Redo changes and become Ready only after the isolated image is decoded, raster-probed, and laid out; creation has an explicit SVG-root or selected-context target; Layers has accessible group disclosure and source-owned Unicode friendly layer names separate from technical IDs; and Delete/Backspace routing follows actual keyboard focus. The larger v0.10.0 Visual Composition Stage 1—multi-selection, moving multiple selected elements, Group/Ungroup, alignment/distribution, basic snapping, and evaluation of safe basic path bounding-box resize—remains planned and is not implemented in Stage 0. Rotation, text resize handles, inline canvas text editing, and path-node editing remain later work. Unsafe or ambiguous operations fail closed. Explicit PNG/PDF **file export**, an installer, and automatic updates are also not provided.
...
| Preview/Layers category | v0.10.0 Stage 0 behavior |
| --- | --- |
...
Text resize handles are deliberately not part of v0.10.0 Stage 0; use the validated `font-size` property instead. The minimum shape dimension is 0.01 SVG user units, and crossing an anchored edge clamps at that value rather than flipping the element.
